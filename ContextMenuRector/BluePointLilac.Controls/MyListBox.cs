...
namespace ContextMenuRector.BluePointLilac.Controls
{
    public class MyListBox : Panel
    {
        ...
        {
            // 使滚动幅度与 MyListItem 的高度相配合，防止滚动过快导致来不及重绘界面变花
    ...
    public class MyList : FlowLayoutPanel
    {
        ...
        public static readonly string CLEAR_FILTER = "--0";

        public void Filter(string keyword)
        {
            if (keyword.Equals(CLEAR_FILTER))
            {
                foreach (MyListItem item in this.Controls)
                {
                    item.Visible = true;
                }
                return;
