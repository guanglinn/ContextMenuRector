...
namespace ContextMenuRector.BluePointLilac.Methods
{
    ...
    {
        /// <summary>RichTextBox 中 ini 语法高亮</summary>
        /// <param name="iniStr">要显示的 ini 文本</param>
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                else if (str.StartsWith("["))
                {
                    if (str.Contains("]"))
                    {
                        ...
                        box.AppendText(str.Substring(index + 1), Color.SkyBlue); // section 标签之后的内容视作注释
                    }
                    else box.AppendText(str, Color.SkyBlue); // section 标签未关闭视作注释
                }
                ...
                else box.AppendText(str, Color.SkyBlue); // 非 section 行和非 key 行视作注释
                if (i != lines.Length - 1) box.AppendText("\r\n");
            }
        }

        /// 引用的源代码：https://archive.codeplex.com/?p=xmlrichtextbox
        /// 仅作简单修改，将原继承类改写为扩展方法
        /// <summary>RichTextBox 中 XML 语法高亮</summary>
        /// <param name="xmlStr">要显示的 XML 文本</param>
        /// <remarks>可直接用 WebBrowser 的 URL 加载本地 XML 文件，但无法自定义颜色</remarks>
