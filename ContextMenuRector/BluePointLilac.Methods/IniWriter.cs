...
namespace ContextMenuRector.BluePointLilac.Methods
{
    public sealed class IniWriter
    {
        ...
        /// <param name="isGetValue">是否是获取 value 值</param>
        private void SetValue(string section, string key, ref string value, bool isGetValue)
        {
            ...
            int sectionRow = -1, keyRow = -1; // 先假设不存在目标 section 和目标 key
            int nextSectionRow = -1; // 下一个 section 的行数
            for (int i = 0; i < lines.Count; i++)
            {
                if (lines[i].StartsWith(sectionLine, StringComparison.OrdinalIgnoreCase))
                {
                    sectionRow = i; break; // 得到目标 section 所在行
                }
            }
            if (sectionRow >= 0) // 如果目标 section 存在
            {
                for (int i = sectionRow + 1; i < lines.Count; i++)
                {
                    ...
                    {
                        nextSectionRow = i; break; // 读取到下一个 section
                    }
                    if (key != null && keyRow == -1)
                    {
                        ...
                        {
                            if (isGetValue) // 如果是获取 Value 值，直接返回
                            {
                                value = lines[i].Substring(index + 1).Trim();
                                return;
                            }
                            keyRow = i; continue; // 得到目标 key 行
                        }
                    }
                }
            }
            ...
            else
            {
                if (keyRow == -1)
                {
                    if (key != null)
                    {
                        // 存在下一个 section 时插入到其上方
                        if (nextSectionRow != -1)
                        {
                            // 目标 section 存在但目标 key 不存在
                            keyRow = nextSectionRow;
                            lines.Insert(keyRow, keyLine);
                        }
                        else
                        {
                            // 不存在下一个 section 则添加到最后
                            lines.Add(keyLine);
                        }
                    }
                    else
                    {
                        // key 为 null 则删除整个 section
                        int count;
        ...
        /// <summary>一次读取只获取一个值，用此方法比 IniReader.GetValue 要快</summary>
