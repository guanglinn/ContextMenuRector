...
using ContextMenuRector.Controls;
using ContextMenuRector.Helper;
...
namespace ContextMenuRector
{
    ...
    {
        private readonly MyToolBarSearchBox searchBox;
        private MyList filterTtarget = null;

        public MainForm()
        {
            this.TopMost = AppConfig.TopMost;
            this.Size = AppConfig.MainFormSize;
            this.Text = AppString.General.AppTitle;
            this.Controls.Add(explorerRestarter);
            ToolBar.AddButtons(ToolBarButtons);
            // Add a search box to the toolbar
            this.searchBox = new MyToolBarSearchBox((sender, e) => Filter(sender, e));
            ToolBar.AddSearchBox(this.searchBox);
        ...
        private Control[] MainControls => new Control[]
        {
            ...
            appSettingBox, languagesBox, dictionariesBox, aboutMeBox, contributorBox, donateBox
        };
        ...
        readonly ContributorBox contributorBox = new ContributorBox();
        ...
        static readonly string[] AboutItems =
        {
            ...
            AppString.SideBar.Contributors,
        ...
        private AsyncDebouncer debouncer = new AsyncDebouncer(TimeSpan.FromMilliseconds(500));
        private void Filter(object sender, EventArgs e)
        {
            if (SideBar.SelectedIndex < 0 || filterTtarget == null)
            {
                return;
            }
            ...
            _ = debouncer.DebounceAsync(async () =>
            {
                // Run search task, filtering items by the keyword
                await Task.Run(() =>
                {
                    if (filterTtarget is MyList list)
                    {
                        list.Filter(keyword);
                    }
                });
        ...
        private void SwitchItem()
        {
            ...
            if (searchBox.TextBox.Text.Trim().Length > 0)
            {
                searchBox.TextBox.Focus();
            }
        }
        ...
        private void SwitchOtherRuleItem()
        {
            switch (SideBar.SelectedIndex)
            {
                ...
                case 7:
                    ...
                    filterTtarget = guidBlockedList;
                    break;
                case 8:
                    iEList.LoadItems();
                    iEList.Visible = true;
                    filterTtarget = iEList;
                    break;
                default:
                    filterTtarget = null;
                    break;
        ...
        private void SwitchAboutItem()
        {
            switch (SideBar.SelectedIndex)
            {
                ...
                case 3:
                    ...
                    break;
                case 4:
                    contributorBox.LoadLanguages();
                    contributorBox.Visible = true;
                    filterTtarget = null;
                    break;
                case 5:
