    ...
    sealed class ShellList : MyList
    {
        ...
        private void AddNewItem(string scenePath)
        {
            ...
            btnEnhanceMenu.MouseDown += (sender, e) =>
            {
                ...
                bool exported = ExternalProgram.ExportRegistry(scenePath, tempPath1);
                using(EnhanceMenusDialog dlg = new EnhanceMenusDialog())
                {
                    dlg.ScenePath = scenePath;
                    dlg.ShowDialog();
                }
                //首次导出失败时已提示过用户，不再第二次导出，也无需比较内容
                if(exported) exported = ExternalProgram.ExportRegistry(scenePath, tempPath2);
                string str1 = File.ReadAllText(tempPath1);
                string str2 = File.ReadAllText(tempPath2);
                File.Delete(tempPath1);
                File.Delete(tempPath2);
                if(exported && !str1.Equals(str2))
                {
            ...
            private void ShowSelectDialog()
            {
                ...
                switch(Scene)
                {
                    ...
                    case Scenes.CustomRegPath:
                        ...
                        frm.Hide();
                        using(Process process = ExternalProgram.StartRegEdit("-m"))
                        {
                            if(process == null) { frm.Show(); return; }//启动失败时已提示用户，恢复主窗口显示
                            process.WaitForExit();
                        }
