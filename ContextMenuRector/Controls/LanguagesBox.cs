...
namespace ContextMenuRector.Controls
{
    sealed class LanguagesBox : FlowLayoutPanel
    {
        ...
        public void LoadLanguages()
        {
            ...
            cmbLanguages.Items.Add("en-US English (default)"); // Default language
            languages.Clear();
            ...
            {
                Dictionary<Label, Control[]> translatorUiItems = new Dictionary<Label, Control[]>();
                foreach (string fileName in Directory.GetFiles(AppConfig.LangsDir, "*.ini"))
                {
                    ...
                    string translatorList = writer.GetValue("General", "Translator");
                    string[] translatorItems = translatorList.Split(new[] { "," }, StringSplitOptions.RemoveEmptyEntries);

                    Dictionary<string, string> translatorDict = new Dictionary<string, string>();
                    // Trace.WriteLine("--- " + fileName.Substring(fileName.LastIndexOf("\\") + 1));
                    foreach (string e in translatorItems)
                    {
                        ...
                        if (mark > 0)
                        {
                            name = item.Substring(1, mark - 1);
                            url = item.Substring(mark + 2, item.Length - mark - 3);
                        }
                        else
                        {
                            name = item;
                        ...
                        // Trace.WriteLine($"{name}: {url}");
                        translatorDict.Add(name, url);
                    }
                    string[] translators = translatorDict.Keys.ToArray();
                    string[] urls = translatorDict.Values.ToArray();
                    ...
                    for (int i = 0; i < translators.Length; i++)
                    {
                        ...
                        if (url.Length > 0)
                        {
                            ...
                            ctrTranslators[i].ForeColor = Color.RoyalBlue;
                    ...
                    translatorUiItems.Add(lblLanguage, ctrTranslators);
                ...
                foreach (var uiItem in translatorUiItems)
                {
                    // Add language label
                    uiItem.Key.Top = top;
                    pnlTranslators.Controls.Add(uiItem.Key);

                    int translatorLabelLeft = left;
                    int gap = 8.DpiZoom();
                    for (int i = 0; i < uiItem.Value.Length; i++)
                    {
                        Label translatorLabel = (Label)uiItem.Value[i];
                        // translatorLabel.BackColor = Color.Pink;
                        if (i > 0) // Add a comma label
                        {
                            ...
                            commaLabel.Location = new Point(translatorLabelLeft - gap.DpiZoom() - 4.DpiZoom(), top);
                            pnlTranslators.Controls.Add(commaLabel);
                            commaLabel.BringToFront();
                        }

                        // Add translator label
                        translatorLabel.Location = new Point(translatorLabelLeft, top);
                        pnlTranslators.Controls.Add(translatorLabel);
                        translatorLabelLeft += translatorLabel.Width + gap.DpiZoom();
                    }
